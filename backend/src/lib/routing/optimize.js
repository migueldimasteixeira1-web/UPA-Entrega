...
// Coordenada da UPA (Av. Vítor Rocha, 410 - Parque Burle, Cabo Frio - RJ,
// 28913-000), geocodificada uma única vez — ponto de partida e retorno de
// toda otimização de rota (issue #73). Nível de precisão de rua (não do
// número exato), suficiente pra distância entre paradas urbanas; refinar
// se algum dia a precisão do número virar relevante.
export const UPA_ORIGIN = { latitude: -22.8833241, longitude: -42.0441878 };
...
// UPA) e uma lista de paradas, devolve a ordem de visita que minimiza o
// deslocamento total. Sem ORS_API_KEY, com menos de 2 paradas geocodificadas,
// ou em caso de falha da API, retorna null — quem chama cai pra ordem FIFO
// (a mesma ordem recebida), nunca bloqueia o despacho.
export async function optimizeDeliverySequence(stops) {
  ...
  const origin = [UPA_ORIGIN.longitude, UPA_ORIGIN.latitude];
  const body = {
