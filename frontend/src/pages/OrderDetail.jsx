    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            ...
            {order.stockReserved && ' · Estoque baixado'}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {order.patientCpf && <p className="text-sm text-slate-500">CPF: {order.patientCpf}</p>}
                  </div>
                </div>
                <div className="flex items-start gap-3">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {order.canRegisterUberFlash && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-upa-800 text-white text-sm font-medium hover:bg-upa-900"
                >
            ...
            {!order.paymentConfirmed && (
              <div className="mb-4 rounded-xl bg-amber-50 border border-amber-100 p-4 text-sm text-amber-800">
            ...
            {order.paymentConfirmed && !order.uberFlashRegistered && order.status === 'FRETE_PAGO' && (
              <div className="mb-4 rounded-xl bg-blue-50 border border-blue-100 p-4 text-sm text-blue-900">
                Pagamento confirmado. Solicite a corrida no Uber Flash e registre o PIN aqui.
              </div>
            )}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className={`rounded-xl border p-4 ${order.deliveryPin ? 'border-blue-100 bg-blue-50/50' : 'border-amber-100 bg-amber-50/50'}`}>
                <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
            ...
            <div className="mt-4 flex flex-wrap gap-3 text-sm">
              <a href={publicUrl} target="_blank" rel="noreferrer" className="text-upa-700 hover:underline inline-flex items-center gap-1">
                Link público do paciente <ExternalLink className="w-3 h-3" />
              </a>
            </div>
          </div>
      ...
      <Modal open={editDelivery} onClose={() => { setEditDelivery(false); setModalError(''); }} title="Registrar dados do Uber Flash">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">PIN do Uber Flash *</label>
            <input
              value={deliveryForm.deliveryPin || ''}
              onChange={(e) => setDeliveryForm({ ...deliveryForm, deliveryPin: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none font-mono"
              placeholder="Código gerado pelo Uber Flash"
            />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Link de rastreio (opcional)</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Observações da entrega (opcional)</label>
            <textarea
